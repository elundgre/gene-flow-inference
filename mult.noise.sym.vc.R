#different noise levels, symmetric, unknown if coalescence rates are same everywhere
...
time_m <- system.time(mult <- mcmapply(mult.small,noise=noise_values,gam=1,const_coal=FALSE,symmetric=TRUE,
                                       seed=rep(seeds,4),mc.cores=6,SIMPLIFY=FALSE))
save.image("mult_noise_sym_vc.RData")
