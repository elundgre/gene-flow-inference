...
a4x4as_com <- run.mcmc(width,height,fixed_g=TRUE,g=g,coal_type=4,gam=gam,const_coal=TRUE,noise=1/500,seed=seed,
                      preburn_iter=preburn_iter,burn_iter=burn_iter,iter=iter,noisy_H=TRUE,type="com")

boxplot(a4x4as_com$ans$g[10*(1:(iter/10)),order(g)],outline=FALSE,main="4x4 Asymmetric Graph (Commute Time Inference)",
...
a4x4as_coal <- run.mcmc(width,height,fixed_g=TRUE,g=g,coal_type=4,gam=gam,const_coal=TRUE,noise=1/500,seed=seed,
                       preburn_iter=preburn_iter_coal,burn_iter=burn_iter_coal,iter=iter_coal,noisy_H=TRUE,type="coal")

boxplot(a4x4as_coal$ans$g[10*(1:(iter_coal/10)),order(g)],outline=FALSE,main="4x4 Asymmetric Graph (Coalescence Time Inference)",
        names=paste0("g",order(g)),xlab="Parameter Index",ylab="Parameter Value (rate)",las=2)
points(g[order(g)],pch=19,col=3)

#put everything in list
a4x4as <- list(coal=a4x4as_coal,com=a4x4as_com,H=H,Hc=Hc,G_adj=G_adj,g=g,gam=gam,burn_iter=burn_iter,burn_iter_coal=burn_iter_coal,
