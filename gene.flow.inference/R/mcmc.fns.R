...
dU_num <- function(lllh_cur,g,gam,G_adj,n,h,sig2ep,lamG,lamgam,delta,ng,ngam,const_coal=TRUE){
  #deprecated function to numerically find derivative
  #use the analytic version below instead (dU_an)
...
make_Deltas <- function(G_adj,n,ng,ngam,const_coal){
  ...
  for(i in 1:ng){
    ...
    Gmod <- as.matrix(Gmod)
    #fill diagonal
    diag(Gmod) <- -Matrix::rowSums(Gmod)
    
    #make product
    GGmod <- Matrix::kronecker(Gmod,diag(n)) + Matrix::kronecker(diag(n),Gmod)
