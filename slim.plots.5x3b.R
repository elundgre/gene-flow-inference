#script for plotting slim stuff with the 5x3 barrier
#move to the directory with output from the slim.infer.5x3b.R (5x3 map with barriers)
...
cairo_pdf(filename=paste0(outpath,"/ind_locs_",fname,".pdf"),width=7,height=(5*(height/width)+2))
  plot.ind.locs(name=name,positions=positions,indiv_s=indiv_s,width=width,heigth=height,
                loc_w=loc_w,loc_h=loc_h,landscape_matrix=landscape_matrix,xlim=xlim,ylim=ylim)
dev.off()
...
#for barrier
colors <- rep(rgb(0,0,0),ng)
colors[c(5,17,8,21,24,37,28,40)] <- rgb(1,0,0)
